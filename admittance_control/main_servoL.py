...
Md = np.diag([50, 50, 500, 50, 50, 50])      # Virtual inertia matrix
Cd = np.diag([100, 100, 100, 50, 50, 50])       # Virtual damping matrix
Kd = np.diag([100, 100, 10000, 5000, 5000, 5000])    # Virtual stiffness matrix
...
# --- Retrieve the Current TCP Pose (Reference Trajectory) ---
# TCP pose is a 6D vector: [x, y, z, rx, ry, rz]
...
target_pose_log = []     # robot target position (nominal_pose + offset)
velocity_log = []        # computed velocity (state[6:12])
reference_pose_log = []  # reference trajectory (nominal_pose, constant here)

# --- Enable Interactive Mode so that all figures remain visible ---
plt.ion()

print("Starting servoL control loop with logging and force filtering. Press Ctrl+C to stop.")
...
try:
    ...
    while True:
        ...
        # Log the computed velocity.
        velocity_log.append(velocity.copy())

        # Log the reference trajectory (nominal_pose remains constant)
        reference_pose_log.append(nominal_pose.copy())
    ...
    for i in range(6):
        axs2[i].plot(time_log, target_pose_log[:, i], label="Target Position " + force_labels[i],
                     color="tab:green")
        axs2[i].plot(time_log, target_pose_log[:, i]-reference_pose_log[:, i], '--', label="Reference " + force_labels[i],
                     color="tab:orange")
